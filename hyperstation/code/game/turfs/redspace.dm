/turf/unsimulated
	name = "unsimulated turf"
	desc = "if you see this, report it"
...
/proc/redspace_abduction(mob/living/target, user)	
	...
	if(!redspace_abduction_z)
		...
		var/datum/space_level/level = SSmapping.add_new_zlevel("Another Time And Place", ZTRAITS_REDSPACE)
		...
		smooth_zlevel(z)
		log_game("Z-level ["Another Time And Place"] loaded at [z]")
	...
	to_chat(user,"<span class='notice'>The mob has been moved. ([ADMIN_LOOKUPFLW(target)])</span>")
